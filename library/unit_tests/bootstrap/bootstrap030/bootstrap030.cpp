...
int faux_main()
{
    ...
    G42CORE_TEST_NS detail::tut_thread<decltype(tut_ss), G42CORE_TEST_NS detail::reporter_outstream> tut_thread1(tut_ss, 0, 0);
    G42CORE_TEST_NS detail::tut_thread<decltype(tut_ss), G42CORE_TEST_NS detail::reporter_outstream>::thread_func(&tut_thread1);

    std::stringstream gtest_ss;
    G42CORE_TEST_NS detail::gtest_thread<decltype(gtest_ss), G42CORE_TEST_NS detail::reporter_outstream> gtest_thread1(gtest_ss, 0, 0);
    G42CORE_TEST_NS detail::gtest_thread<decltype(gtest_ss), G42CORE_TEST_NS detail::reporter_outstream>::thread_func(&gtest_thread1);

    std::stringstream cpputest_ss;
    G42CORE_TEST_NS detail::cpputest_thread<decltype(cpputest_ss), G42CORE_TEST_NS detail::reporter_outstream> cpputest_thread1(cpputest_ss, 0, 0);
    G42CORE_TEST_NS detail::cpputest_thread<decltype(cpputest_ss), G42CORE_TEST_NS detail::reporter_outstream>::thread_func(&cpputest_thread1);

    std::cout << tut_ss.str();
    std::cout << gtest_ss.str();
    std::cout << cpputest_ss.str();

    return tut_thread1.result() | gtest_thread1.result() | cpputest_thread1.result();
}
...
extern "C" JNIEXPORT jint JNICALL Java_grail42_core_bootstrap030_FauxConsole_main(JNIEnv * env, jobject  obj)
{
